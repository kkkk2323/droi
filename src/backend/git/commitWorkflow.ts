...
export async function detectGitTools(params: { projectDir: string }): Promise<GitToolsInfo> {
  ...
  if (!projectDir)
    return { hasGh: false, hasFlow: false, prTool: null, prDisabledReason: 'Missing projectDir' }
...
async function gitCommitWithMessage(params: {
  projectDir: string
  message: string
}): Promise<string> {
...
async function gitMergeIntoBranch(params: {
  projectDir: string
  sourceBranch: string
  targetBranch: string
}): Promise<void> {
...
async function flowCreatePr(params: {
  projectDir: string
  baseBranch: string
  headBranch: string
  title: string
...
export async function commitWorkflow(
  ...
  if (workflow !== 'commit' && workflow !== 'commit_push' && workflow !== 'commit_push_pr')
