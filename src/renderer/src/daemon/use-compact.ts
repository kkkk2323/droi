// `/compact` in the Daemon is a handoff: it summarises the conversation into a
// new Session whose file records the old one as its parent, and answers with
// the new id. The Daemon does not list that link, so the Client tags the child
// itself; every Client then chains the two (see sessions.ts).
...
export interface CompactActions {
  /** Resolves to the child Session's id, or null when the Daemon refused. */
