...
- Exception: if a mission-verification follow-up explicitly requires fresh **same-session** destructive proof (for example VAL-CROSS-004 or VAL-CROSS-005), copied snapshots, injected state, restored-session reselection, and disk logs are supporting context only. The claimed proof must still show one live Electron Mission session traversing the required destructive transition end to end.
