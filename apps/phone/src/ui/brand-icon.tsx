// A model's brand mark, as the web Client draws it (brand-icon.tsx there):
// colour marks as they are; OpenAI, xAI and Moonshot in the text colour.
import type { Brand } from '@droi/daemon-layer/model-brand'
import { Sparkles } from 'lucide-react-native'
import { View } from 'react-native'
import Svg, { Path, SvgXml } from 'react-native-svg'
import * as marks from './brand-marks'

const COLOR_MARKS: Partial<Record<Brand, string>> = marks

export function BrandIcon({ brand, size, color }: { brand: Brand; size: number; color: string }) {
  const xml = COLOR_MARKS[brand]
  const mark = xml ? (
    <SvgXml xml={xml} width={size} height={size} />
  ) : brand === 'openai' ? (
    <Svg viewBox="0 0 24 24" width={size} height={size}>
      <Path
        fill={color}
        d="M9.205 8.658v-2.26c0-.19.072-.333.238-.428l4.543-2.616c.619-.357 1.356-.523 2.117-.523 2.854 0 4.662 2.212 4.662 4.566 0 .167 0 .357-.024.547l-4.71-2.759a.797.797 0 00-.856 0l-5.97 3.473zm10.609 8.803v-5.399c0-.333-.143-.57-.428-.737L13.415 7.85l1.95-1.118a.484.484 0 01.5 0l4.543 2.617c1.309.76 2.188 2.378 2.188 3.948 0 1.808-1.07 3.472-2.782 4.163zM7.802 12.61l-1.95-1.142a.446.446 0 01-.237-.428V5.804c0-2.545 1.95-4.472 4.59-4.472 1 0 1.928.334 2.712.928L8.23 4.973a.821.821 0 00-.428.737v6.9zM12 15.036l-2.795-1.57v-3.33L12 8.567l2.796 1.57v3.33L12 15.035zm2.212 7.634c-.999 0-1.927-.333-2.712-.928l4.686-2.712a.822.822 0 00.428-.737v-6.9l1.974 1.142a.446.446 0 01.238.428v5.236c0 2.545-1.975 4.471-4.614 4.471zm-6.827-6.85a.484.484 0 01-.5 0L2.34 13.201C1.032 12.443.152 10.825.152 9.255c0-1.832 1.094-3.472 2.807-4.163v5.423c0 .333.142.57.428.737l5.947 3.45-1.95 1.117zm-.238 6.017c-2.688 0-4.71-2.021-4.71-4.567 0-.19.024-.38.048-.57l4.686 2.711a.797.797 0 00.856 0l5.97-3.448v2.26c0 .19-.072.333-.238.428l-4.543 2.616c-.619.357-1.356.523-2.117.523zm.238-1.712a5.962 5.962 0 004.162 1.713 5.947 5.947 0 005.827-4.757c2.665-.69 4.378-3.188 4.378-5.733 0-1.666-.713-3.283-1.998-4.448.119-.5.19-1 .19-1.498 0-3.401-2.759-5.947-5.946-5.947-.642 0-1.26.095-1.88.31A5.962 5.962 0 0010.205 0a5.947 5.947 0 00-5.827 4.757C1.713 5.447 0 7.945 0 10.49c0 1.666.713 3.283 1.998 4.448-.119.5-.19 1-.19 1.499 0 3.401 2.759 5.946 5.946 5.946.642 0 1.26-.095 1.88-.309a5.96 5.96 0 004.162 1.713z"
      />
    </Svg>
  ) : brand === 'moonshot' ? (
    <Svg viewBox="0 0 24 24" width={size} height={size}>
      <Path
        fill="#1783FF"
        d="M21.846 0a1.923 1.923 0 110 3.846H20.15a.226.226 0 01-.227-.226V1.923C19.923.861 20.784 0 21.846 0z"
      />
      <Path
        fill={color}
        d="M11.065 11.199l7.257-7.2c.137-.136.06-.41-.116-.41H14.3a.164.164 0 00-.117.051l-7.82 7.756c-.122.12-.302.013-.302-.179V3.82c0-.127-.083-.23-.185-.23H3.186c-.103 0-.186.103-.186.23V19.77c0 .128.083.23.186.23h2.69c.103 0 .186-.102.186-.23v-3.25c0-.069.025-.135.069-.178l2.424-2.406a.158.158 0 01.205-.023l6.484 4.772a7.677 7.677 0 003.453 1.283c.108.012.2-.095.2-.23v-3.06c0-.117-.07-.212-.164-.227a5.028 5.028 0 01-2.027-.807l-5.613-4.064c-.117-.078-.132-.279-.028-.381z"
      />
    </Svg>
  ) : brand === 'xai' ? (
    <Svg viewBox="0 0 24 24" width={size} height={size}>
      <Path
        fill={color}
        d="M6.469 8.776L16.512 23h-4.464L2.005 8.776H6.47zm-.004 7.9l2.233 3.164L6.467 23H2l4.465-6.324zM22 2.582V23h-3.659V7.764L22 2.582zM22 1l-9.952 14.095-2.233-3.163L17.533 1H22z"
      />
    </Svg>
  ) : (
    <Sparkles size={size} color={color} strokeWidth={1.75} />
  )
  // Decorative next to the brand's name, so hidden from screen readers; the
  // test id is how the tests find a mark that has no name.
  return (
    <View testID={`brand-${brand}`} aria-hidden style={{ width: size, height: size }}>
      {mark}
    </View>
  )
}
