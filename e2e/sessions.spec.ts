...
test.describe('sidebar', () => {
  ...
  test('is keyboard navigable', async ({ page, openClient }) => {
    await openClient()
    const first = page.getByRole('button', { name: /Refactor billing/ })
    await first.focus()
...
test.describe('session history', () => {
  ...
  test('a reload keeps the open Session', async ({ page, openClient }) => {
    await openClient()
    await page.getByRole('button', { name: /Fix the login bug/ }).click()
...
test.describe('long history', () => {
  ...
  test('is virtualised and opens scrolled to the latest message', async ({ page, openClient }) => {
    await openClient()
    await page.getByRole('button', { name: /Long chat/ }).click()
    const transcript = page.getByRole('log', { name: 'Transcript' })
